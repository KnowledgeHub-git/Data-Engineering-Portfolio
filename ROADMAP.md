# Portfolio Roadmap

## Execution Status

| Phase | Project | Status | Started | Completed |
|-------|---------|--------|---------|-----------|
| 1 - Data Eng | 01 - Multi-Domain Data Mesh | Complete | 2026-06-13 | 2026-06-13 |
| 1 - Data Eng | 02 - Real-Time Weather Pipeline | Complete | 2026-06-13 | 2026-06-13 |
| 1 - Data Eng | 03 - Economic Data Vault | Complete | 2026-06-13 | 2026-06-13 |
| 2 - ML | 04 - Stock Price Forecasting | Not Started | — | — |
| 2 - ML | 05 - Healthcare Network Analysis | Not Started | — | — |
| 2 - ML | 06 - Climate Risk Scoring | Not Started | — | — |
| 3 - GenAI | 07 - Earnings Call RAG | Not Started | — | — |
| 3 - GenAI | 08 - Patent Intelligence | Not Started | — | — |
| 3 - GenAI | 09 - Financial Document QA | Not Started | — | — |
| 4 - Agents | 10 - Market Research Agent | Not Started | — | — |
| 4 - Agents | 11 - Climate ESG Agent | Not Started | — | — |
| 4 - Agents | 12 - Real Estate Advisor Agent | Not Started | — | — |
| 5 - Qlik | 13 - Qlik KPI Suite | Not Started | — | — |
| 5 - Qlik | 14 - Qlik AutoML Pipeline | Not Started | — | — |
| 5 - Qlik | 15 - GitHub Developer Analytics | Not Started | — | — |

## Recommended Priority

1. **Project 1** — Foundation; everything else depends on clean dimensional models
2. **Project 7** — Fastest GenAI demo (existing transcript data, Cortex Search)
3. **Project 4** — Cortex ML showcase with minimal setup
4. **Project 10** — Combines 1 + 4 + 7 into an autonomous agent
5. **Project 13** — Plug Qlik into gold layers as they materialize

## Key Milestones

- [x] Gold layer operational (Project 1 complete)
- [x] Weather pipeline with Dynamic Tables + Alerts (Project 2 complete)
- [ ] First ML model deployed to Model Registry (Project 4)
- [ ] First Cortex Search service live (Project 7)
- [ ] First Cortex Agent functional (Project 10)
- [ ] Qlik connected end-to-end (Project 13)
